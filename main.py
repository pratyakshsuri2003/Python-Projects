    ...
    if chfovapo(position) == True:
        ...
        if spacefree(position) == True:
            ...
            if chfowin(player) == True:
                if player == 'X':
                    winner = Label(text=f"{xNameVariable.get()} wins")
                    winner.grid(row=4,column=0,columnspan=3)
                    isWinner = 1
                else:
                    winner = Label(text=f"{oNameVariable.get()} wins")
                    winner.grid(row=4,column=0,columnspan=3)
                    isWinner = 1
            elif chfodra() == True:
...
playerDetailsLabel = Label(text="Enter player details:")
playerDetailsLabel.grid(row=10 , column=0 , columnspan=3)

# playerDetailsVariable 

xNameVariable = StringVar()
oNameVariable = StringVar()

# Entry Widgets

xPlayerLabel = Label(text="Enter name for Player X")
xPlayerLabel.grid(row=20 , column=0 , columnspan=3)
xPlayerEntry = Entry(textvariable=xNameVariable)
xPlayerEntry.grid(row=20 , column=3)

oPlayerLabel = Label(text="Enter name for Player O")
oPlayerLabel.grid(row=30 , column=0, columnspan=3)
oPlayerEntry = Entry(textvariable=oNameVariable)
oPlayerEntry.grid(row=30 , column=3)
